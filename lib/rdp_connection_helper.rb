module Intrigue
  module Ident
    module RdpConnectionHelper
      ...
      # TODO: TOOLS - TO BE MOVED
      def rand_text_generator(length, char = 'A')
        char * (length.is_a?(Range) ? length.first : length)
      end
      ...
      def initialize(info = {})
        ...
        register_options(
          [
            ...
            OptString.new('RDP_CLIENT_NAME',
                          [false, 'The client computer name to report during connect, UNSET = random', 'rdesktop']),
            OptString.new('RDP_DOMAIN', [false, 'The client domain name to report during connect']),
            OptAddress.new('RDP_CLIENT_IP',
                           [true, 'The client IPv4 address to report during connect', '192.168.0.100']),
            Opt::RPORT(3389)
      ...
      def rdp_connect
        self.rdp_sock = connect_tcp(false)
        # self.rdp_sock = connect_tcp(false)
        rdp_sock.setsockopt(::Socket::IPPROTO_TCP, ::Socket::TCP_NODELAY, 1)
      end

      def rdp_disconnect
        # TODO: disconnect socketry
        # disconnect(rdp_sock)
        self.rdp_sock = nil
      end
      ...
      def rdp_recv(length = -1, timeout = 5)
        res = rdp_sock.get_once(length, timeout)
      ...
      def rdp_fingerprint
        ...
        return true, peer_info unless [RDPConstants::PROTOCOL_HYBRID,
                                       RDPConstants::PROTOCOL_HYBRID_EX].include? server_selected_proto
      ...
      def rdp_dispatch_loop
        rdp_handle_packet(rdp_recv) while rdp_sock
      end

      def rdp_create_channel_msg(_chan_user_id, chan_id, data, flags = 3, data_length = nil)
        data_length ||= data.length

        pdu = [
          [25 << 2].pack('C'), # MCS send data request structure, choice 25
          [rdp_user_id, chan_id].pack('S>S>'), # MCS send data request structure, choice 25
      ...
      def rdp_check_protocol(req_proto = RDPConstants::PROTOCOL_SSL)
        # We wont have a user here
        # @user_name = datastore['RDP_USER'] || Rex::Text.rand_text_alpha(7)

        # @domain = datastore['RDP_DOMAIN'] || Rex::Text.rand_text_alpha(7)

        # @computer_name = datastore['RDP_CLIENT_NAME'] || Rex::Text.rand_text_alpha(15)

        # @ip_address = datastore['RDP_CLIENT_IP']

        @user_name = rand_text_generator(7)

        @domain = rand_text_generator(7)

        @computer_name = rand_text_generator(15)
        ...
        if res
          ...
          if ['SSL_NOT_ALLOWED_BY_SERVER', 'Negotiation Response packet too short.'].include? err_msg
            ...
            if res
              ...
              return true, RDPConstants::PROTOCOL_RDP if err_msg == 'Negotiation Response packet too short.'
      ...
      def rdp_negotiate_security(channels, req_proto = RDPConstants::PROTOCOL_SSL)
        if req_proto == RDPConstants::PROTOCOL_SSL
          swap_sock_plain_to_ssl
          rdp_send_recv(pdu_connect_initial(channels, req_proto, @computer_name))
        ...
        self.rdp_user_id = res[9, 2].unpack1('n')

        # send channel requests
        [1009, 1003, 1004, 1005, 1006, 1007, 1008].each do |chan|
          rdp_send_recv(pdu_channel_join_request(rdp_user_id, chan))
        end
      ...
      def rdp_generate_license_keys(data)
        ...
        [client_random, license_key, license_sign_key]
      end
      ...
      def rdp_parse_negotiation_response(data)
        ...
        response_code = data[11].unpack1('C')

        if response_code == 2 # TYPE_RDP_NEG_RSP
          # RDP Negotiation Response - 2.2.1.2.1
          server_selected_proto = data[15..18].unpack1('L<')

          proto_label = RDPConstants::RDP_NEG_PROTOCOL[server_selected_proto]
          return server_selected_proto, nil if proto_label

          [nil, "Unknown protocol in Negotiation Response: #{server_selected_proto}"]

        elsif response_code == 3 # TYPE_RDP_NEG_FAILURE
          # RDP Negotiation Failure - 2.2.1.2.2
          failure_code = data[15..18].unpack1('L<')
          [nil, RDPConstants::RDP_NEG_FAILURE[failure_code]]
        else
          [nil, "Unknown Negotiation Response code: #{response_code}"]
        end
      end
      ...
      def rdp_parse_connect_response(pkt)
        ...
        while ptr < rdp_pkt.length
          ...
          header_length = rdp_pkt[ptr + 2..ptr + 3].unpack1('S<')
          ...
          if header_type == "\x02\x0c"
            ...
            bitlen = rdp_pkt[ptr + 72..ptr + 75].unpack1('L<') - 8
      ...
      def rdp_calculate_rc4_keys(client_random, server_random)
        ...
        [initialClientEncryptKey128, initialClientDecryptKey128, mac_key, sessionKeyBlob]
      end
      ...
      def bytes_to_bignum(bytes_val, order = 'little')
        ...
        bytes = bytes.scan(/../).reverse.join('') if order == 'little'
        s = '0x' + bytes
        s.to_i(16)
      end
      ...
      def swap_sock_plain_to_ssl
        ...
        ssl = OpenSSL::SSL::SSLSocket.new(rdp_sock, ctx)
        ...
        rdp_sock.extend(Rex::Socket::SslTcp)
        rdp_sock.sslsock = ssl
        rdp_sock.sslctx  = ctx
      end
      ...
      def per_data(*ds)
        ...
        if data.length < 0x4000
          [data.length | 0x8000].pack('S>') + data
        else
          "\xA2" + [data.length].pack('S>') + data
        end
      end
      ...
      def rdp_handle_packet(pkt)
        if pkt && pkt[0] == "\x03" && (pkt[4..6] == "\x02\xf0\x80") && (pkt[7] == "\x68")
          chan_user_id = pkt[8..9].unpack1('S>')
          chan_id = pkt[10..11].unpack1('S>')
          flags = pkt[18..21].unpack1('<L')
      ...
      def rdp_on_channel_receive(pkt, chan_user_id, chan_id, flags, data)
        ctype = data[0..1].unpack1('S')

        if ctype == RDPConstants::RDPDR_CTYP_CORE
          opcode = data[2..3].unpack1('S')
      ...
      def rdp_on_core_server_capability(_pkt, chan_user_id, chan_id, _flags, data)
      ...
      def rdpdr_client_device_list_announce_request(_pkt, chan_user_id, chan_id, _flags, _data)
        reply = [
      ...
      def rdpdr_client_announce_reply(_pkt, chan_user_id, chan_id, _flags, _data)
        reply = [
          RDPConstants::RDPDR_CTYP_CORE,
      ...
      def rdpdr_client_name_request(_pkt, chan_user_id, chan_id, _flags, _data)
      ...
      attr_accessor :rdp_sock, :rdp_user_id
