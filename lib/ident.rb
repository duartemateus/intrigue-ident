...
require_relative 'rdp_connection_helper'
require_relative 'mongodb_connection_helper'
require_relative 'opt_base'
require_relative 'opt_bool'
require_relative 'rdp_connection_service'
...
##################################
# Load in rdp matchers and checks
##################################
require_relative 'rdp/matchers'
include Intrigue::Ident::Rdp::Matchers

require_relative 'rdp/check_factory'
require_relative '../checks/rdp/base'

# rdp fingerprints
check_folder = File.expand_path('../checks/rdp', File.dirname(__FILE__)) # get absolute directory
Dir["#{check_folder}/*.rb"].each { |file| require_relative file }
###
    ...
    class Ident
      ...
      def list_checks(path = '[URI]')
        Intrigue::Ident::Http::CheckFactory.checks.map do |x|
          ...
          Intrigue::Ident::Rdp::CheckFactory.checks.map { |x| x.new.generate_checks }
        ).flatten
      end
      ...
      def fingerprint_service(ip_address_or_hostname, port, opts = {})
        ...
        if port == 3389 || port =~ /^\d+3389$/
          ident_matches = generate_rdp_request_and_check(ip_address_or_hostname) || {}
        end
        ###
