...
##################################
# Load in ntp matchers and checks
##################################
require_relative 'ntp/matchers'
include Intrigue::Ident::Ntp::Matchers

require_relative 'ntp/check_factory'
require_relative '../checks/ntp/base'

# ntp fingerprints
check_folder = File.expand_path('../checks/ntp', File.dirname(__FILE__)) # get absolute directory
Dir["#{check_folder}/*.rb"].each { |file| require_relative file }
    ...
    class Ident
      ...
      def list_checks(path = '[URI]')
        Intrigue::Ident::Http::CheckFactory.checks.map do |x|
          ...
          Intrigue::Ident::Ntp::CheckFactory.checks.map { |x| x.new.generate_checks }
        ).flatten
      end
      ...
      def fingerprint_service(ip_address_or_hostname, port, opts = {})
        ...
        if port == 123 || port =~ /^\d+123$/
          ident_matches = generate_ntp_request_and_check(ip_address_or_hostname) || {}
        end
        ###
