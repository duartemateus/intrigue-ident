  ...
  class OptBase
    ...
    def initialize(in_name, attrs = [],
      ...
      if attrs.is_a?(String) || attrs.length == 0
        ...
        self.default = if default.nil? && enums.length > 0
                         enums[0]
                       else
                         default
                       end
      ...
      self.desc += " Max parameter length: #{max_length} characters" unless max_length.nil?
    ...
    def valid?(value, check_empty: true)
      if check_empty && required? && (value.nil? || value.to_s.empty?)
        # required variable not set
        return false
      end
      return !!value.match(regex) if regex && !value.nil?
    ...
    def invalid_value_length?(value)
      value.length > max_length if !value.nil? && !max_length.nil?
    end
